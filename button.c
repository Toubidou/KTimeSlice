...
BUTTON *button_create(int (*detect_func)(void), void (*short_evt)(void))
{
    ...
    BUTTON *btn_new = (BUTTON *)malloc(sizeof(BUTTON));
    ...
    btn_new->cob_trigger_flag = 0;
    list_append(&btn_list_head, &(btn_new->next));
    button_num++;
...
int button_cob_Reg(uint8_t btn_id0, uint8_t btn_id1, void (*cob_event)(void))
{
    ...
    if (bci.cob_num < BUTTON_COB_NUM_MAX)
    {
        ...
        list_for_each(node, &btn_list_head)
        {
            temp_btn_p = list_entry(node, BUTTON, next);
